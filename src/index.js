...
const compileEclairProgram = async (program) =>
  withFile(async ({ path: inputFile }) => {
    ...
    const { stderr } = await execFile('./compile_eclair.sh', [inputFile, wallocObjFile, outputFile]);
...
const main = () => {
  ...
  app.post('/', async (req, res) => {
    ...
    const { errorMessage, wasmFile, cleanup } = await compileEclairProgram(program);
    if (errorMessage) {
